 ...
 *	1)static inline const Vector2D<T> fromPolar(float r, float theta)			*
 *	The function takes r and theta as arguments and converts them from			*
 *	Polar to x y cartesian coordinates.											*
 *	Usage Ex: Vector2D<float>::fromPolar(2,PI/3.0);								*
 *	2)static inline float angle(const Vector2D<T>& head, const Vector2D<T>&tail)*
 *	Returns the angle made by the vector joining 2 Vectors/Points (head & tail) *
 *	in the range -pi to pi														*
 *	Usage Ex: Vector2D<float>::angle(p1,p2); //where p1 and p2 are of type		*
 *	Vector2D<float>																*
 *	3)static inline float dist(const Vector2D<T>& v1, const Vector2D<T>& v2)	*
 *	Returns the Eucledian distance between the 2 vectors						*
 *	4)static inline float distSq(const Vector2D<T>& v1, const Vector2D<T>& v2)	*
 *	Returns square of Eucledian distance between the 2 vectors					*
 *	5)inline T dot(const Vector2D<T>& rhs)										*
 *	Returns dot product of calling vector with another vector					*
 *	Usage Ex: Vector2D<int> v1(2,3), v2(3,-4),v1_dot_v2;						*
 *	v1_dot_v2 = v1.dot(v2);														*/
#pragma once
#ifndef GEOMETRY_H
#define GEOMETRY_H
